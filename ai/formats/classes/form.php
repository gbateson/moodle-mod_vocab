...
class form extends \mod_vocab\aiform {
    ...
    public function definition() {
        ...
        // By default, the "Add a new format" section is expanded,
    ...
    /**
     * Format config settings for a format.
     *
     * @param object $config
    ...
    public function format_config($config, $actions=[], $showowner=false, $showownerpic=false) {
        ...
        // Format the formatp text to show only the 1st 20 chars and the final 20 chars.
        $name = 'formattext';
        if (isset($config->$name)) {
            $label = $this->get_string($name).$labelsep;
            $label = \html_writer::tag('dt', $label, $dt);
